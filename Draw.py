...
def algorithm_menu():
    ...
    while True:
        ...
        for i, algorithm in enumerate(algo_names):
            drawMenu(
                algorithm,
                (myEnvironment.size[0] / 2, myEnvironment.size[1] / 2 + 50 + 50 * i),
                i == selected,
            )
...
def drawLevel(matrix_to_draw):
    ...
    wall = pygame.image.load(
        myEnvironment.getPath() + "/themes/" + "/images/wall.png"
    ).convert()
    box = pygame.image.load(
        myEnvironment.getPath() + "/themes/" + "/images/box.png"
    ).convert()
    box_on_target = pygame.image.load(
        myEnvironment.getPath() + "/themes/" + "/images/box_on_target.png"
    ).convert()
    space = pygame.image.load(
        myEnvironment.getPath() + "/themes/" + "/images/space.png"
    ).convert()
    target = pygame.image.load(
        myEnvironment.getPath() + "/themes/" + "/images/target.png"
    ).convert()
    player = pygame.image.load(
        myEnvironment.getPath() + "/themes/" + "/images/player.png"
    ).convert()
