...
def bfs_solve(level):
    ...
    def try_move(matrix, position, direction):
        ...
        # move box
            ...
            if (
                0 <= x + 2 * dx < len(new_matrix[y])
                and 0 <= y + 2 * dy < len(new_matrix)
                and new_matrix[y + 2 * dy][x + 2 * dx] in [" ", ".", "+"]
            ):
                new_matrix[y][x] = " " if new_matrix[y][x] != "+" else "."
                new_matrix[y + dy][x + dx] = "@" if next_cell != "*" else "+"
                new_matrix[y + 2 * dy][x + 2 * dx] = (
                    "$" if new_matrix[y + 2 * dy][x + 2 * dx] == " " else "*"
                )
                return new_matrix, next_pos, True
    ...
    while queue:
        ...
        for direction in ["L", "R", "U", "D"]:
            new_matrix, new_position, valid_move = try_move(
                current_matrix, current_position, direction
            )
            if valid_move:
